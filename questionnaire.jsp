...
{
{{"identification"}, {"nelem","1"},{"musthave","0"},{"group","0"},{"gminmax","1.1"},{"gplace","0"},{"gorder","0"},{"gsequence","random"}},
{{"asianblackiden"},{"choices","rate1"},{"stem","Which statement best describes you?"}},
};                 
ArrayList <String[][][]> scales = new ArrayList <String[][][]> ();
  scales.add(identification);
...
<script language="JavaScript" type="text/javascript" src="/implicit/common/en-us/js/task.js"></script>
   ...
   <style type="text/css">
      ...
      .med {font-size: 16px; font-weight: bold}
       .medblue {font-size: 20px; line-height:150%;  color:#000000;}
       .medblue2 {font-size: 24px; line-height:150%;  color:#000000;}
      .sml {font-size: 10px;}
...
// #BDEDFF light blue
 ...
 function makeItem(curitem) {
  preclass="medblue";
   if ((curitem==0) || (curitem==5)) preclass="medblue2";
   
   
  // var pctcompleted= Math.round(((curitem)/150)*100);
 ...
 // var minselect = js_minselect[curitem];
  ...
  var prefixskip = '<button type="button" style="font-size:10; font-weight: bold; width: 120 px; background-color:';
  var curdisplay='<table width="70%" align=center><tr><td><font color="#AAAAAA"><span class=small><b>Section:'+(curitem+1)+'/'+js_nitem+'</b></span></font><br /><span  class='+preclass+'>'+js_prefix[curitem]+'</span><br/><span class=big style="background-color: #FFFFCC">'+js_stem[curitem]+'</span><br/><br/></td></tr></table> <table width="70%" align=center cellpadding="2">';
    ...
    curdisplay+='<tr><td>';
    curdisplay+='<input type=text style="font-size:16;" length=120 maxlength=120 size=120 id="text1" name="text1" onkeypress="keyCode=(event.which)?event.which:event.keyCode;return (keyCode != 13); " onkeydown="keyCode=(event.which)?event.which:event.keyCode;return (keyCode != 13); "> <br/><br/>';
    
    curdisplay+='<button type="button" id="thisbut" style="font-size:16; font-weight: bold; width: 100px; background-color:#EEEEEE" onmouseup=\'changestate(3,"text1",2,"thisbut")\'>Next</button></td></tr>';
  ...
  // the finished button
   ...
   if (maxselect>1) {
    ...
    curdisplay+='<tr><td  class="sml" colspan="'+icols+'"><br>Select an item by clicking on it and turning it <u>BLUE</u>.<br/>To unselect, click it once more.<br>When you are done selecting the required number of items (as stated in the question) you can click on the &ldquo;Finished and continue&rdquo; button.<br/>The Finished button will turn <u>YELLOW</u> (it will not change color if required number of items are not selected) and you can confirm by clicking it a second time.<br/><br/></td></tr>';
}
   
  
   if ((curitem<1) && (maxselect<=1)) curdisplay+='<tr><td colspan="'+icols+'">Selecting an answer once colors it yellow.<br/> You can change your answer by selecting another option. <br /> To confirm, click the selected (yellow) button a second time.<br/><br/></td></tr>';
 ...
 if (next!=prev) mainitem.innerHTML=makeItem(gnext);    else mainitem.innerHTML=makeItem(gnext);
    ...
    if (<%=testing%>==123) window.location="index.htm";
